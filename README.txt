...
To perform the test, simply run the errors.py file, preferably using a python 2.7 interpreter. The following parameters are required: '-d' followed by a path to the desired testing dataset, and '-m' followed by a path to the model file. Optionally, a -c argument followed by an npz file of the pre-clustered dataset can be provided if one wishes to use a custom clusterisation. The data set needs to contain information on atomic positions, forces and energies (data['R'],data['F'],data['E']).
...
After successfully running errors.py, a folder named '{name of model file}_{name of dataset file}' will appear in the '/storage' directory, containing a png file of a column graph of the average force prediction error on every cluster, as well as an xyz file for all data points in each respective cluster in order of the highest error (with 'Cluster50.xyz' having the largest average error if using default parameters).

If the argument -e is present, energy prediction errors will be calculated rather than force prediction errors. 

Optionally, to redraw the graph for an already computed file, one can generate a graph using:
python errors.py -g <storage file> 
